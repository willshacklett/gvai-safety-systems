from __future__ import annotations

from gvai.effects import make_action
from gvai.effect_gate import GVEffectGate
from gvai.effect_invariants import make_effect_invariant
from gvai.runtime_guard_v2 import GVRuntimeGuardV2
from gvai.runtime_policy import GVRuntimePolicy
from gvai.sentinel import GVSentinel


def build_guard() -> GVRuntimeGuardV2:
    invariant = make_effect_invariant(
        name="safety_integrity",
        description="Protected safety mechanisms cannot be bypassed.",
        forbidden_effects={
            "disables_gate",
            "changes_invariant",
            "executes_outside_boundary",
        },
    )

    return GVRuntimeGuardV2(
        sentinel=GVSentinel(),
        policy=GVRuntimePolicy(
            GVEffectGate([invariant])
        ),
    )


def main() -> None:
    guard = build_guard()

    normal = make_action(
        "normal_step",
        {"normal_operation"},
    )

    recovery = make_action(
        "recovery_step",
        {"low_impact"},
    )

    destabilize = make_action(
        "increase_load",
        {"destabilizing"},
    )

    # Phase 1: progressively deteriorating state.
    deterioration = [
        [1.00, 1.00, 1.00, 1.00],
        [1.00, 1.01, 0.99, 1.00],
        [1.00, 1.03, 0.97, 1.00],
        [1.00, 1.06, 0.94, 1.00],
        [1.00, 1.10, 0.90, 1.00],
        [1.00, 1.16, 0.84, 1.00],
        [1.00, 1.24, 0.76, 1.00],
        [1.00, 1.34, 0.66, 1.00],
        [1.00, 1.46, 0.54, 1.00],
        [1.00, 1.60, 0.40, 1.00],
    ]

    # Phase 2: simulated stabilization/recovery.
    recovery_trajectory = [
        [1.00, 1.52, 0.48, 1.00],
        [1.00, 1.42, 0.58, 1.00],
        [1.00, 1.32, 0.68, 1.00],
        [1.00, 1.23, 0.77, 1.00],
        [1.00, 1.16, 0.84, 1.00],
        [1.00, 1.10, 0.90, 1.00],
        [1.00, 1.06, 0.94, 1.00],
        [1.00, 1.03, 0.97, 1.00],
        [1.00, 1.01, 0.99, 1.00],
        [1.00, 1.00, 1.00, 1.00],
    ]

    trajectory = (
        [("stress", values) for values in deterioration]
        + [("recovery", values) for values in recovery_trajectory]
    )

    print(
        f"{'STEP':>4} | "
        f"{'PHASE':>8} | "
        f"{'STATUS':>12} | "
        f"{'RECOV':>7} | "
        f"{'VAR':>8} | "
        f"{'VEL':>8} | "
        f"{'ACCEL':>8} | "
        f"{'NORMAL':>7} | "
        f"{'RECOVERY':>8} | "
        f"{'DESTAB':>7}"
    )
    print("-" * 112)

    for phase, values in trajectory:
        observation = guard.observe(values)

        normal_decision = guard.evaluate_action(
            normal,
            observation,
        )

        recovery_decision = guard.evaluate_action(
            recovery,
            observation,
        )

        destabilize_decision = guard.evaluate_action(
            destabilize,
            observation,
        )

        print(
            f"{observation.step:>4} | "
            f"{phase:>8} | "
            f"{observation.status:>12} | "
            f"{observation.recoverability_score:>7.3f} | "
            f"{observation.variance_value:>8.4f} | "
            f"{observation.variance_velocity:>8.4f} | "
            f"{observation.variance_acceleration:>8.4f} | "
            f"{str(normal_decision.allowed):>7} | "
            f"{str(recovery_decision.allowed):>8} | "
            f"{str(destabilize_decision.allowed):>7}"
        )


if __name__ == "__main__":
    main()
