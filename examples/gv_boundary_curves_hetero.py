...
OUT_CSV = ROOT / "gv_boundary_curves_hetero.csv"
OUT_PNG = ROOT / "gv_boundary_curves_hetero.png"
...
def init_gains(base_gain: float, n: int = 8, spread: float = 0.02) -> List[float]:
    return [base_gain + random.uniform(-spread, spread) for _ in range(n)]


def evolve(values: List[float], gains: List[float], noise: float) -> List[float]:
    mu = mean(values)
    out: List[float] = []
    for v, g in zip(values, gains):
        out.append(mu + (v - mu) * g + random.uniform(-noise, noise))
    return out


def mix_ws_partial(values: List[float], p_mix: float = 0.45) -> List[float]:
    out = values[:]
    n = len(values)
    for i in range(n):
...
def delayed_update(values: List[float], buffers: List[List[float]], gains: List[float], noise: float) -> List[float]:
    mu = mean(values)
    out: List[float] = []
    for i, v in enumerate(values):
        ...
        g = gains[i]
        updated = mu + (source - mu) * g + random.uniform(-noise, noise)
        out.append(updated)
    return out
...
def classify_case(base_gain: float, delay: int, noise: float = 0.05, steps: int = 30) -> dict:
    ...
    values = init_vals()
    gains = init_gains(base_gain, n=len(values), spread=0.02)
    ...
    for step in range(1, steps + 1):
        values = delayed_update(values, buffers, gains=gains, noise=noise)

        if step > 10:
            values = mix_ws_partial(values, p_mix=0.45)
...
def plot_curves(rows: List[dict], delays: List[int], path: Path) -> None:
    ...
    ax.set_title("GV Boundary Curves with Heterogeneity + Partial Coupling")
    ax.legend()
